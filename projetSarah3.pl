...
acquisition_prop_type1(Abi,Abi1,Tbox) :- 
    ...
    nl,read(C),
    concept(C), nl,replace(C,RC),nnf(not(RC),NRC),
    concat([((I,NRC))],Abi,Abi1),
    nl,write("la Abox apres ajout " + Abi1).

acquisition_prop_type2(Abi,Abi1,Tbox) :- 
    nl,write("entrer la propositon à demontrer en commençant par le 1er concept"),
    nl,read(C1),
...
complete_some([(I,some(R,C))|Lie],Lpt,Li,Lu,Ls,Abr) :- 
    ...
    resolution(Lie1, Lpt1, Li1, Lu1, Ls1,[instR(I,B,R)|Abr]).
...
deduction_all(Lie,[(I,all(R,C))|Lpt],Li,Lu,Ls,[(I,X,R)|Abr]) :-
    (member(instR(I,_,R),Abr) -> true, L=1 ),
    continue(L).
